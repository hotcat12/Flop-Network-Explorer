...
import { getActiveIdentity, getSessionDid } from "@/lib/did";
import { sendTechnocoreMessage, type SendDiagnostic } from "@/lib/technocore";
...
export default function ClubCommunity() {
  ...
  async function sendMessage(event: FormEvent) {
    ...
    try {
      ...
      const result = await sendTechnocoreMessage(ROOM, cleanNick, clean, identity);
      ...
      const failed = err as Error & { diagnostic?: SendDiagnostic };
      if (failed.diagnostic) setDiagnostic(failed.diagnostic);
  ...
  return <ExplorerShell eyebrow="CLUB COMMUNITY">
    <div className="mb-7 flex flex-wrap items-end justify-between gap-5"><div><div className="font-mono text-[10px] uppercase tracking-[.25em] text-fuchsia-300">Public room / live chat</div><h1 className="mt-3 text-4xl font-bold tracking-[-.05em] text-white sm:text-6xl">Club Community</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-zinc-400">A public Technocore room. Read the public Technocore room. Sending is DID-signed only, so every accepted message is attributable to its signing key.</p></div><a href={`https://technocore.chat/r/${ROOM}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border border-cyan-300/25 px-4 py-3 font-mono text-[10px] uppercase tracking-widest text-cyan-200 hover:bg-cyan-300/10">Open source <ExternalLink className="h-3.5 w-3.5" /></a></div>
