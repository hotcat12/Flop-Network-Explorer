...
export async function parseImportedPem(raw: string): Promise<DidIdentity> { const match = raw.match(/-----BEGIN PRIVATE KEY-----([\s\S]+?)-----END PRIVATE KEY-----/); if (!match) throw new Error("Unsupported PEM. Choose an Ed25519 PKCS#8 identity.pem file with BEGIN PRIVATE KEY."); const binary = atob(match[1].replace(/\s/g, "")); const der = Uint8Array.from(binary, (char) => char.charCodeAt(0)); const key = await crypto.subtle.importKey("pkcs8", der as unknown as BufferSource, { name: "Ed25519" }, true, ["sign"]); const jwk = await crypto.subtle.exportKey("jwk", key); return { did: didFromJwk(jwk), jwk }; }
