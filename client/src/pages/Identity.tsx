...
export default function Identity() {
  ...
  useEffect(() => {
    const refreshSession = () => {
      ...
      if (sessionDid && active?.did === sessionDid) { setIdentity(active); setUnlockOnly(false); setDidInput(sessionDid); setStatus(`Signed in as ${sessionDid}.`); }
      else {
        if (sessionDid) { setSessionDid(null); window.dispatchEvent(new Event("flop:identity")); }
        setIdentity(null); setUnlockOnly(hasSavedIdentity()); setDidInput(""); setStatus(hasSavedIdentity() ? "Logged out or refreshed. The encrypted Digital ID remains here; enter its password to unlock it." : "No identity in this browser yet. Paste your seed to sign in.");
      }
    };
    refreshSession();
  ...
  async function signInFromSeed() {
    ...
    try {
      ...
      setActiveIdentity(next); setSessionDid(next.did); setIdentity(next); setUnlockOnly(false); setDidInput(next.did); window.dispatchEvent(new Event("flop:identity"));
  ...
  async function create() {
    ...
    setBusy(true);
    try { const next = await generateIdentity(); await saveIdentity(next, password); setActiveIdentity(next); setSessionDid(next.did); setIdentity(next); setUnlockOnly(false); setDidInput(next.did); window.dispatchEvent(new Event("flop:identity")); setStatus("New identity encrypted locally and signed in."); }
  ...
  async function importJwk() {
    ...
    setBusy(true);
    try { const next = parseImportedJwk(importText); await saveIdentity(next, password); setActiveIdentity(next); setSessionDid(next.did); setIdentity(next); setUnlockOnly(false); setDidInput(next.did); window.dispatchEvent(new Event("flop:identity")); setImportText(""); setStatus(`Signed in as ${next.did}. Existing identity encrypted locally.`); }
  ...
  function logout() { setActiveIdentity(null); setSessionDid(null); setIdentity(null); setUnlockOnly(hasSavedIdentity()); setDidInput(""); setSeedText(""); setPassword(""); setConfirmPassword(""); setStatus("Logged out. Your encrypted Digital ID remains in this browser and can be unlocked with its password."); window.dispatchEvent(new Event("flop:identity")); }
  function forget() { clearIdentity(); logout(); setStatus("Local Digital ID and DID session deleted from this browser."); }
  ...
  return <ExplorerShell eyebrow="IDENTITY VAULT">
    <div className="mx-auto max-w-3xl">
      ...
      {identity ? <section className="hud-card border-cyan-300/25 bg-cyan-300/5 p-6 shadow-[0_0_45px_rgba(34,211,238,.08)]"><div className="font-mono text-[10px] uppercase tracking-widest text-cyan-200/70">Signed in and ready to post</div><div className="mt-3 break-all font-mono text-sm text-white">{identity.did}</div><p className="mt-3 text-xs leading-5 text-zinc-400">Your private key is active only in this browser tab. Technocore receives signed messages, never the key.</p><button onClick={logout} className="mt-6 w-full border border-amber-300/40 bg-amber-300/10 px-4 py-4 text-xs font-bold uppercase tracking-wider text-amber-100 hover:bg-amber-300/20">Log out</button></section> : unlockOnly && hasSavedIdentity() ? <section className="hud-card border-cyan-300/25 p-6 shadow-[0_0_45px_rgba(34,211,238,.08)]"><div className="font-mono text-[10px] uppercase tracking-widest text-cyan-200/70">Encrypted identity saved</div><p className="mt-3 text-sm leading-6 text-zinc-400">Your seed is hidden after logout. Enter only the local password to unlock this browser’s encrypted identity.</p><label className="mt-5 block font-mono text-[10px] uppercase tracking-widest text-zinc-500">Local password<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter your local password" className="mt-2 w-full border border-white/15 bg-black/30 px-3 py-3 text-sm text-zinc-200 outline-none focus:border-cyan-300/50" /></label><button onClick={() => void unlock()} disabled={busy || password.length < 8} className="mt-5 w-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-4 text-xs font-bold uppercase tracking-wider text-cyan-100 disabled:opacity-40">{busy ? "Unlocking…" : "Unlock saved identity"}</button>{status && <div className="mt-4 border border-white/10 bg-white/[.03] p-4 text-xs leading-5 text-zinc-300">{status}</div>}</section> : <section className="hud-card overflow-hidden border-cyan-300/25 shadow-[0_0_45px_rgba(34,211,238,.08)]"><div className="border-b border-white/10 bg-cyan-300/5 px-6 py-5"><div className="flex items-center gap-3"><KeyRound className="h-5 w-5 text-cyan-300" /><div><h2 className="text-xl font-bold text-white">Unified identity sign-in</h2><p className="mt-1 text-xs text-zinc-500">No Shift key or keyboard shortcut is required.</p></div></div></div><div className="space-y-5 p-6"><label className="block font-mono text-[10px] uppercase tracking-widest text-zinc-500">Manual 64-character seed<textarea value={seedText} onChange={(event) => setSeedText(event.target.value)} rows={3} placeholder="Paste 64 hexadecimal characters" className="mt-2 w-full border border-white/15 bg-black/30 px-3 py-3 font-mono text-xs text-zinc-200 outline-none focus:border-cyan-300/50" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="block font-mono text-[10px] uppercase tracking-widest text-zinc-500">Local password<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="At least 8 characters" className="mt-2 w-full border border-white/15 bg-black/30 px-3 py-3 text-sm text-zinc-200 outline-none focus:border-cyan-300/50" /></label><label className="block font-mono text-[10px] uppercase tracking-widest text-zinc-500">Confirm password<input type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Type it again" className="mt-2 w-full border border-white/15 bg-black/30 px-3 py-3 text-sm text-zinc-200 outline-none focus:border-cyan-300/50" /></label></div><button onClick={() => void signInFromSeed()} disabled={busy || !seedText.trim()} className="inline-flex w-full items-center justify-center gap-2 bg-cyan-300 px-4 py-4 text-xs font-bold uppercase tracking-wider text-black disabled:cursor-not-allowed disabled:opacity-40">{busy ? "Checking seed…" : "Encrypt locally and sign in"}</button><p className="text-center text-[11px] leading-5 text-zinc-500">Your password encrypts the identity in localStorage. The seed is never uploaded.</p>{hasSavedIdentity() && <button onClick={() => void unlock()} disabled={busy || password.length < 8} className="w-full border border-cyan-300/25 px-4 py-3 text-xs font-bold uppercase tracking-wider text-cyan-200 disabled:opacity-40">Unlock saved identity</button>}{status && <div className="border border-white/10 bg-white/[.03] p-4 text-xs leading-5 text-zinc-300">{status}</div>}</div></section>}
