...
export default function Identity() {
  ...
  useEffect(() => {
    const refreshSession = () => {
      ...
      if (sessionDid && active?.did === sessionDid) { setIdentity(active); setDidInput(sessionDid); setStatus(`Signed in as ${sessionDid}.`); }
      else {
        if (sessionDid) { setSessionDid(null); window.dispatchEvent(new Event("flop:identity")); }
        setIdentity(null); setDidInput(""); setStatus(hasSavedIdentity() ? "Logged out or refreshed. The encrypted Digital ID remains here; enter its password to unlock it." : "No identity in this browser yet. Paste your seed to sign in.");
      }
    };
    refreshSession();
  ...
  function logout() { setActiveIdentity(null); setSessionDid(null); setIdentity(null); setDidInput(""); setSeedText(""); setPassword(""); setConfirmPassword(""); setStatus("Logged out. Your encrypted Digital ID remains in this browser and can be unlocked with its password."); window.dispatchEvent(new Event("flop:identity")); }
